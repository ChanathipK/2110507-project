...
export default function CreateHotelPane() {
    ...
    const createNormal = (
        <div className="w-96 h-[30rem] bg-slate-500 rounded-lg flex flex-col px-8 py-4">
            <h1 className="text-slate-50 text-2xl text-center mt-2">Hotel Information</h1>
            <form className="w-full mt-4">
                ...
                <div className="w-full flex justify-center items-center">
                    <input className="bg-orange-200 px-12 py-1 mt-2 rounded-md hover:cursor-pointer duration-150 hover:text-slate-50 hover:bg-orange-500" type="submit" value="Create Hotel" onClick={(e) => {
                            e.preventDefault();
                            createHotel();
                        }} />
                    </div>
    ...
    const createSuccess = (
        <div className="w-96 h-[30rem] bg-slate-500 rounded-lg flex flex-col justify-center items-center px-8 py-4">
            <h1 className="text-xl text-slate-50 text-center">Insert Hotel Data <span className="text-green-300">Successfully.</span></h1>
            <h2 className="text-center text-slate-50"><button className="text-blue-300" onClick={(e) => {
                ...
                setTel("");
                setImageURL("");
            }}>Click here</button> to insert another hotel information</h2>
        </div>
    );

    const createFail = (
        <div className="w-96 h-[30rem] bg-slate-500 rounded-lg flex flex-col justify-center items-center px-8 py-4">
            <h1 className="text-xl text-center text-slate-50">Insert Hotel Information <span className="text-red-300">Failed.</span></h1>
            <h2 className="text-slate-50"><button className="text-blue-300" onClick={(e) => {
                e.preventDefault();
                location.reload();
    ...
    async function createHotel() {
        if (
            ...
            let authToken = "";
            if (userSession != null) {
                authToken = userSession.user.token;
            }
